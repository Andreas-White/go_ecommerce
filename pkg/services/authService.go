...
func (s *AuthService) ChangePassword(ctx context.Context, userID string, changePasswordDTO *models.ChangePasswordDTO) error {
	if changePasswordDTO.CurrentPassword == "" || changePasswordDTO.NewPassword == "" {
		return utils.HandleServiceErrors(ctx, errors.New("current_password and new_password are required"), "service/ChangePassword")
	}
	if len(changePasswordDTO.NewPassword) < 8 {
		return utils.HandleServiceErrors(ctx, errors.New("new_password must be at least 8 characters long"), "service/ChangePassword")
	}
	if changePasswordDTO.CurrentPassword == changePasswordDTO.NewPassword {
		return utils.HandleServiceErrors(ctx, errors.New("new password cannot be the same as the current password"), "service/ChangePassword")
	}

	auth, err := s.AuthRepo.GetAuthByUserID(ctx, userID)
	if err != nil {
		if err == sql.ErrNoRows {
			return utils.HandleServiceErrors(ctx, sql.ErrNoRows, "service/ChangePassword")
		}
	...
	if !utils.CheckPasswordHash(changePasswordDTO.CurrentPassword, auth.Password) {
		return utils.HandleServiceErrors(ctx, errors.New("incorrect current password"), "service/ChangePassword")
	}
